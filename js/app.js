...
const App = (() => {
  ...
  function init() {
    ...
    // App.init() is NOT called directly — auth.js calls _bootAfterAuth after login
  }

  /* ── BOOT AFTER AUTH (called by auth.js) ── */
  function _bootAfterAuth() {
    ...
    Habits.render();
    Cal.render();
    // Patch STORE to auto-sync after auth
    try { CloudSync.patchStore(); } catch(e){}
  }

  return {
    init, goTo, computeStats, showToast, STORE,
    _renderChecklist, _renderTodayStats, _refreshTodayProgress,
    _toggleItem, resetToday, _tMins, _bootAfterAuth,
  };
})();

/* ── LOGOUT ── */
App.logout = async function() {
  if (!confirm('Logout? Your data is saved in the cloud and will restore on next login.')) return;
  try {
    if (window._fbSignOut) await window._fbSignOut();
    if (window._SYNC) window._SYNC.stop();
...
App.syncNow = function() {
  ...
  setTimeout(() => {
    ...
    App.showToast('☁️ Data synced to cloud ✓');
